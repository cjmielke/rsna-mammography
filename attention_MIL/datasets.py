...
def getTensorFileDataset(args):
    ...
    tensorFiles = glob(f'/fast/rsna-breast/features/{args.tileSize}/{args.encoder}/*/*.pt')
    ...
    if args.exclude123:
        print(f'Size before easy exclusion : {labelDF.shape}')
        easyCases = pd.read_csv('/fast/rsna-breast/tables/correct123.csv')
        labelDF = labelDF[~labelDF.image_id.isin(easyCases.image_id)]
        print(f'Size after easy exclusion : {labelDF.shape}')
        #sys.exit()

    if args.exclude_stage2:
        print(f'Size before easy exclusion : {labelDF.shape}')
        easyCases = pd.read_csv('/fast/rsna-breast/tables/correct_stage2.csv')
        labelDF = labelDF[~labelDF.image_id.isin(easyCases.image_id)]
        print(f'Size after easy exclusion : {labelDF.shape}')
        #sys.exit()
...
class EmbeddingDataset(Dataset):
    ...
    def getTensor(self, row, item):
        ...
        tensorFile = f'/fast/rsna-breast/features/{self.args.tileSize}/{self.args.encoder}/{row.patient_id}/{row.image_id}.pt'

        T = torch.load(tensorFile, map_location=torch.device('cpu')).detach()
    ...
    def __getitem__(self, item):
        ...
        if self.noise:
            ...
            tensor += self.noise * torch.randn(tensor.shape)#.cuda()
        #cancer = row.cancer*1.0
        if self.args.metadata:
            laterality = 1.0 if row.laterality == 'L' else 0.0
            age = row.age if row.age is not None else 50.0
            if np.isnan(age): age=50.0
            age = age-50
            age = age/50.0
            return tensor, cancer, (age, laterality, row)
...
def collate(batch):
    ...
    #print(labels)
    if len(batch[0]) == 3:        # metadata is returned
        ages = torch.from_numpy(np.asarray([float(item[2][0]) for item in batch]).astype('float32'))
        laterality = torch.from_numpy(np.asarray([float(item[2][1]) for item in batch]).astype('float32'))
        rows = pd.DataFrame([item[2][2] for item in batch])       # dataframe
        #print(rows.shape)
        #sys.exit()
        return embeddings, labels, ages, laterality, rows
