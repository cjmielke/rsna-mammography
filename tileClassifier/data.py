...
'''
OLD attention sets, before the contamination in the dataset was cleaned uo
    ...
    if attnset == 'kind':
        ...
        attenDF1 = pd.read_feather('/fast/rsna-breast/tables/attn_scores_all_classic_sweep_9.feather')
        attenDF2 = pd.read_feather('/fast/rsna-breast/tables/attn_scores_all_kind_sweep_53.feather')
        def norm(df):
            df['raw'] = df.raw - df.raw.min()
            df['raw'] = df.raw / df.raw.max()
            return df
        attenDF1 = norm(attenDF1)
        attenDF2 = norm(attenDF2)
        attenDF = attenDF1
        attenDF['raw'] = attenDF1.raw + attenDF2.raw


'''


def getNewAttnsWithRaws(attnThresh=None, rawSigma=None, attnset='classy', target='cancer'):
    # has 5,251,862 tiles !


    #if attnset == 'apricot':
    #    attenDF = pd.read_feather('/fast/rsna-breast/tables/attn_scores_apricot_sweep_45.feather')
    #else:
    #    raise ValueError
    ...
    # shouldn't need this fix anymore
    # remove missing tiles :(
    #currentTiles = pd.read_feather('/fast/rsna-breast/tables/tile_224_stats_sorted.feather')

    #currentTiles['ID'] = currentTiles.apply(lambda r: (r.imgID, r.row, r.col), axis=1)
    #attenDF['ID'] = attenDF.apply(lambda r: (r.imgID, r.row, r.col), axis=1)
    print(f'attentionset before filtering {attenDF.shape}')
    #attenDF = attenDF[attenDF.ID.isin(set(currentTiles.ID))]
    print(f'attentionset after filtering {attenDF.shape}')


    labels = pd.read_csv('/fast/rsna-breast/train.csv')[['image_id', 'cancer','age','BIRADS','density','biopsy']]
    labels['age'] = labels.age.fillna(labels.age.mean())
    labels['age'] = (labels.age.round(-1)/10.0).astype(int)
    labels['density'] = labels.density.fillna(0).replace(dict(A=1,B=2,C=3,D=4))
    #labels = labels.fillna(-1)

    attenDF = attenDF.merge(labels, left_on='imgID', right_on='image_id')
    attenDF = attenDF.fillna(-1)
    ...
    if target == 'cancer':
        ...
        thresh = (attenDF.raw.mean() + float(rawSigma) * float(attenDF.raw.std()))
        attenDF['target'] = 1.0*(attenDF.raw > thresh)
    ...
    attenDF['rawScore'] = attenDF.raw-attenDF.raw.mean()        # zero center
...
class CollateFunction(nn.Module):
    ...
    def __init__(self, argparse, validation=False):
        super().__init__()
        self.args = argparse
        input_size = 224
        min_scale = 0.8
    ...
    def forward(self, batch: List[tuple]):
        ...
        biopsy = torch.from_numpy(
            np.asarray([item[2]['biopsy'] for item in batch]).astype(np.float32)
        )

        #age = np.asarray([item[2]['age'] for item in batch]).astype(np.float32)
        age = torch.from_numpy(np.asarray([item[2]['age'] for item in batch])).long()
        birads = torch.from_numpy(np.asarray([item[2]['BIRADS'] for item in batch])).long()
        density = torch.from_numpy(np.asarray([item[2]['density'] for item in batch])).long()

        rawAttn = np.asarray([item[2]['rawScore'] for item in batch]).astype(np.float32)
        ...
        #print(labels)
        return transforms, labels, rawAttn, age, birads, density, biopsy
